...
export async function main() {
  ...
  if (!scheduleFaSheetNames.includes("Schedule FA (Phase 1)")) {
    throw new Error(`Expected a Schedule FA (Phase 1) sheet, found: ${scheduleFaSheetNames.join(", ")}`);
  }
  ...
  if ((withoutForeignAccountsSheets as Array<{ sheet: string }>).some((s) => s.sheet === "Schedule FA (Phase 1)")) {
    throw new Error("A workbook with no foreign accounts entered should not have a Schedule FA sheet at all.");
  }
