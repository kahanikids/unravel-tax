...
export function InsurancePolicyPanel({
  ...
  function updatePolicy(id: string, patch: Partial<InsurancePolicy>) {
    onChangePolicies(policies.map((policy) => (policy.id === id ? { ...policy, ...patch } : policy)));
  }
  ...
  return (
    <section className="supplemental-form">
      <p className="step-lede">
        Add one card per policy to compute whether its Section 10(10D) exemption survives, and if not, the taxable
        amount - the aggregate-premium check on the dashboard only checks a single combined premium figure, not each
        policy's own issue date and history. <RuleSourceLink refs={insuranceRule.source_refs} />
      </p>

      {policies.length === 0 ? (
        <p className="checklist-empty">No policies added yet.</p>
      ) : (
        summary.results.map(({ policy, exempt, reason, taxableAmount, taxTreatment, estimatedTax }) => (
          <div className="insurance-policy-card" key={policy.id}>
            <div className="supplemental-grid">
              <label className="supplemental-field">
                ...
                <select
                  ...
                  onChange={(event) => updatePolicy(policy.id, { policyType: event.target.value as InsurancePolicy["policyType"] })}
                >
              ...
              <label className="supplemental-field">
                <input
                  ...
                  onChange={(event) => updatePolicy(policy.id, { isDeathBenefit: event.target.checked })}
                />{" "}
                ...
                <>
                  <label className="supplemental-field">
                    ...
                    <input
                      ...
                      onChange={(event) => updatePolicy(policy.id, { issueDate: event.target.value })}
                    />
                  </label>
                  <label className="supplemental-field">
                    Sum assured
                    <input
                      ...
                      placeholder="₹0"
                      onChange={(event) => updatePolicy(policy.id, { sumAssured: Number(event.target.value) || 0 })}
                    />
                  </label>
                  <label className="supplemental-field">
                    ...
                    <input
                      ...
                      placeholder="₹0"
                      onChange={(event) => updatePolicy(policy.id, { annualPremium: Number(event.target.value) || 0 })}
                    />
                  </label>
                  <label className="supplemental-field">
                    ...
                    <input
                      ...
                      placeholder="₹0"
                      onChange={(event) => updatePolicy(policy.id, { totalPremiumsPaidToDate: Number(event.target.value) || 0 })}
                    />
                  </label>
                  <label className="supplemental-field">
                    ...
                    <input
                      ...
                      placeholder="₹0"
                      onChange={(event) => updatePolicy(policy.id, { maturityPayoutThisYear: Number(event.target.value) || 0 })}
                    />
                  </label>
                </>
              ) : null}
            ...
            <div className="regime-result">
              <p className="regime-verdict">{TAX_TREATMENT_LABELS[taxTreatment]}: {reason}</p>
              {!exempt ? (
      ...
      {summary.totalOtherSourcesSlabIncome > 0 ? (
        <p className="step-lede">
          ₹{formatAmount(summary.totalOtherSourcesSlabIncome)} of taxable traditional-policy payout is added
          automatically to your slab income in the regime comparison below - don't re-enter it in "Bank interest &amp;
          other income".
        </p>
      ) : null}
      {summary.totalUlipCapitalGainsTax > 0 ? (
        <p className="step-lede">
          Each taxable ULIP above uses the full ₹1,25,000 annual LTCG exemption on its own. If you also have other
          equity long-term gains this year, the two share one exemption combined, so your real combined tax may be
          higher than the per-policy figures shown here. A CA should combine both under the one limit before you file.
        </p>
      ) : null}
    </section>
  );
