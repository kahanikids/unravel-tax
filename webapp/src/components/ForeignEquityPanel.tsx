...
export function ForeignEquityPanel({
  ...
  function updateHolding(id: string, patch: Partial<ForeignEquityHolding>) {
    onChangeHoldings(
      holdings.map((holding) => (holding.id === id ? { ...holding, ...patch } : holding))
    );
  }
  ...
  function addHolding() {
    onChangeHoldings([
      ...holdings,
      { ...BLANK_FOREIGN_EQUITY_HOLDING, id: newForeignEquityHoldingId() }
    ]);
  }
  ...
  return (
    <section className="supplemental-form">
      <p className="step-lede">
        Foreign shares (including RSU/ESPP once vested) are taxed like <strong>unlisted</strong>{" "}
        Indian shares, not listed equity: long-term only past{" "}
        {shareRule.long_term_holding_period_days_gt} days (24 months), at a flat{" "}
        {(shareRule.ltcg_rate * 100).toLocaleString("en-IN")}% with no indexation; short-term is
        your slab rate, added to the regime comparison's other income.{" "}
      ...
      {holdings.length === 0 ? (
        ...
        summary.results.map(
          ({ holding, sold, gain, taxTreatment, estimatedLtcgTax, foreignTaxCreditOnGain }) => (
              ...
              <div className="supplemental-grid">
                <label className="supplemental-field">
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      updateHolding(holding.id, { entityName: event.target.value })
                    }
                  />
                </label>
                <label className="supplemental-field">
                  <input
                    ...
                    onChange={(event) =>
                      updateHolding(holding.id, { isRsuOrEspp: event.target.checked })
                    }
                  />{" "}
                ...
                <label className="supplemental-field">
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      updateHolding(holding.id, { acquisitionDate: event.target.value })
                    }
                  />
                </label>
                <label className="supplemental-field">
                  ...
                  <input
                    ...
                    placeholder="₹0"
                    onChange={(event) =>
                      updateHolding(holding.id, { costBasisInr: Number(event.target.value) || 0 })
                    }
                  />
                </label>
                {holding.isRsuOrEspp ? (
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        updateHolding(holding.id, {
                          perquisiteValueInr: Number(event.target.value) || 0
                        })
                      }
                    />
                  </label>
                ...
                <label className="supplemental-field">
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      updateHolding(holding.id, {
                        closingValueInr: Number(event.target.value) || 0
                      })
                    }
                  />
                </label>
                <label className="supplemental-field">
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      updateHolding(holding.id, { saleDate: event.target.value })
                    }
                  />
                </label>
                <label className="supplemental-field">
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      updateHolding(holding.id, {
                        saleProceedsInr: Number(event.target.value) || 0
                      })
                    }
                  />
                </label>
                <label className="supplemental-field">
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      updateHolding(holding.id, {
                        foreignTaxPaidOnGainInr: Number(event.target.value) || 0
                      })
                    }
                  />
                </label>
                  ...
                  <>
                    ...
                    {taxTreatment === "long_term" ? (
                      ...
                      <p className="regime-verdict">
                        Short-term - taxed at your slab rate, added to the regime comparison's other
                        income below, not shown as a flat amount here.
                      </p>
                    )}
                  </>
                ) : null}
      ...
      {summary.totalLtcgTax > 0 ? (
        <p className="step-lede">
          Total foreign-share long-term capital gains tax:{" "}
          <strong>₹{formatAmount(summary.totalLtcgTax)}</strong>, shown as its own row in the CA
          Summary above.
        </p>
      ) : null}
      {summary.totalStcgGain > 0 ? (
        <p className="step-lede">
          ₹{formatAmount(summary.totalStcgGain)} of short-term foreign-share gains is added
          automatically to your slab income in the regime comparison below.
        </p>
      ) : null}
      {summary.totalPerquisiteValueInr > 0 ? (
        <p className="step-lede">
          ₹{formatAmount(summary.totalPerquisiteValueInr)} of RSU/ESPP perquisite value is added
          automatically to your salary income in the regime comparison below - don't re-enter it in
          the salary field.
        </p>
      ) : null}
      ...
      <div className="regime-result">
        <label className="supplemental-field">
          Foreign tax paid on dividends/interest/short-term foreign-share gains/RSU perquisite
          combined (₹)
          <input
            type="number"
            min={0}
        ...
        {hasRegimeResult ? (
          <p className="regime-verdict">
            The Section 90/91 foreign tax credit estimate on this slab-taxed foreign income
            (including any foreign dividend/interest from the Schedule FA accounts section) shows as
            its own CA Summary row above, using the average-rate method (Rule 128) against whichever
            regime looks cheaper. This is a planning estimate, not a Form 67 number - Form 67 needs
            a per-country breakdown a CA should verify.
          </p>
        ) : (
