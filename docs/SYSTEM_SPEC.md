...
**A. Template Workbook(s)** — Google Sheets as the primary format (free, browser-only, no install, works on a phone). An Excel export is provided as an alternative for users who prefer it or work offline, generated by the same underlying structure. One base template with modular tabs; category-specific tabs are added or removed depending on the user's profile (see Section 6).
...
unravel-tax/
  ...
  notebooks/
    build-workbook.ipynb   <- Colab-ready port of this session's Python scripts (Tier 2, ships before the webapp does)
  templates/              <- unchanged, Section 12 repo structure
  prompts/                <- unchanged
```
